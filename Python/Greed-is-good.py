...
URL to Challenge: https://www.codewars.com/kata/5270d0d18625160ada0000e4
...
def score(dice):
    ...
    for x in range(0,6):
        if roll[x] > 2:
            points += (1000 if x == 0 else 100) * (x + 1)
            roll[x] -= 3
        if (x == 0 or x == 4) and (roll[x] > 0):
                points += (100 if x == 0 else 50) * (roll[x])
        print(roll)
    return points

print(score([6, 6, 6, 2, 2]))